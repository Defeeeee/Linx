...
function(PRINT_BOARD_LIST)
    foreach (PLATFORM ${ARDUINO_PLATFORMS})
        if (${PLATFORM}_BOARDS)
            ...
            print_list(STATUS ${PLATFORM}_BOARDS)
            message(STATUS "")
        endif ()
    endforeach ()
endfunction()
...
function(PRINT_PROGRAMMER_LIST)
    foreach (PLATFORM ${ARDUINO_PLATFORMS})
        if (${PLATFORM}_PROGRAMMERS)
            ...
            print_list(STATUS ${PLATFORM}_PROGRAMMERS)
        endif ()
        message(STATUS "")
    endforeach ()
endfunction()
...
function(find_arduino_libraries VAR_NAME SRCS ARDLIBS)
    ...
    set(ARDUINO_LIBS)
    foreach (SRC ${SRCS})
        message(DEBUG "Checking SRC ${SRC}")
                ...
                if ("#${SRC_LINE}#" MATCHES "^#[ \t]*#[ \t]*include[ \t]*[<\"]([^>\"]*)[>\"]#")
                    ...
                    foreach (LIB_SEARCH_PATH ${include_dirs} ${LIBRARY_SEARCH_PATH} ${ARDUINO_LIBRARIES_PATH} ${${ARDUINO_PLATFORM}_LIBRARIES_PATH} ${CMAKE_CURRENT_SOURCE_DIR} ${CMAKE_CURRENT_SOURCE_DIR}/libraries ${ARDUINO_EXTRA_LIBRARIES_PATH})
                        if (EXISTS ${LIB_SEARCH_PATH}/${INCLUDE_NAME}/${CMAKE_MATCH_1})
                            message(DEBUG "Found dep ${LIB_SEARCH_PATH}/${INCLUDE_NAME}/${CMAKE_MATCH_1}")
                            list(APPEND ARDUINO_LIBS ${LIB_SEARCH_PATH}/${INCLUDE_NAME})
                            break()
                        endif ()
                        if (EXISTS ${LIB_SEARCH_PATH}/${INCLUDE_NAME}/src/${CMAKE_MATCH_1})
                            message(DEBUG "Found dep ${LIB_SEARCH_PATH}/${INCLUDE_NAME}/src/${CMAKE_MATCH_1}")
                            list(APPEND ARDUINO_LIBS ${LIB_SEARCH_PATH}/${INCLUDE_NAME})
                            break()
                        endif ()
                        ...
                        if ((EXISTS ${LIB_SEARCH_PATH}/${CMAKE_MATCH_1}) OR ${_header_generated})
                            message(DEBUG "Found dep ${LIB_SEARCH_PATH}/${CMAKE_MATCH_1} OR ${_header_generated}")
...
# vsch: Listing recurse is no longer needed for known libraries or others.
#     However, since since I did not validate that all of them have src/ subdirectory, leaving it as is.
...
function(setup_arduino_library VAR_NAME BOARD_ID LIB_PATH COMPILE_FLAGS LINK_FLAGS)
    ...
    if (NOT TARGET ${TARGET_LIB_NAME})
        ...
        if (NOT DEFINED ${LIB_SHORT_NAME}_RECURSE)
            # detect if library has src subdirectory with a file named same as the library and source or header extension
            set(${LIB_SHORT_NAME}_RECURSE False)

            # vsch: Instead of having to specify RECURSE, see if there is a file with library name in src/ subdirectory
            #       of the library root. If it has src/ with source file(s) with library name,
            #       then set recurse for it but only src/ subdirectory.
            string(REGEX REPLACE ".*/" "/src" LIB_TEST_PATH ${LIB_NAME})
            set(FILE_SEARCH_LIST
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.cpp
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.c
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.cc
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.cxx
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.h
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.hh
                    ${LIB_PATH}/src/${LIB_TEST_PATH}.hxx)
            message(DEBUG "FILE_SEARCH_LIST:")
            print_list(DEBUG FILE_SEARCH_LIST)
            message(DEBUG "")
            file(GLOB LIB_SRCS_TEST ${FILE_SEARCH_LIST})
            message(DEBUG "LIB_SRCS_TEST:")
            print_list(DEBUG LIB_SRCS_TEST)
            message(DEBUG "")
            if (LIB_SRCS_TEST)
                message(STATUS "Setting: ${LIB_SHORT_NAME}_RECURSE True")
                set(${LIB_SHORT_NAME}_RECURSE True)
            endif ()
        endif ()
        ...
        if (LIB_SRCS)
            ...
            message(DEBUG "LIB_DEPS:")
            print_list(DEBUG LIB_DEPS)
            message(DEBUG "")
...
#                   vsch: any files in a directory starting with cmake-build or examples
#                         will be excluded from the list because these are either CMake
#                         build directories and will contain sources and example files which
#                         may have C/C++ source/header extensions
#
...
function(find_sources VAR_NAME LIB_PATH RECURSE)
    ...
    if (RECURSE)
        ...
        # vsch: there may be a more efficient way to filter out files, but CMake is not my forte
        set(LIB_FILES)
        foreach (FILE_ITEM ${FILE_LIST})
            if (FILE_ITEM MATCHES ".*/((cmake-build[^/]*)|(examples[^/]*))/.*$")
                message(DEBUG "Removing ${FILE_ITEM} from library source files.")
            else ()
                set(LIB_FILES ${LIB_FILES} ${FILE_ITEM})
            endif ()
        endforeach ()
    else ()
...
# [PRIVATE/INTERNAL]
#
# print_list(MESSAGE_TYPE SETTINGS_LIST)
#
#      SETTINGS_LIST - Variables name of settings list
#      MESSAGE_TYPE - DEBUG/STATUS/... or any of the others
#
# Print list settings and names (see load_arduino_syle_settings()).
#=============================================================================#
function(PRINT_LIST MESSAGE_TYPE SETTINGS_LIST)
    if (${SETTINGS_LIST})
        set(MAX_LENGTH 0)
        ...
        foreach (ENTRY_NAME ${${SETTINGS_LIST}})
            ...
            foreach (X RANGE ${PADDING_LENGTH})
                set(PADDING "${PADDING} ")
            endforeach ()
            message(${MESSAGE_TYPE} "   ${PADDING}${ENTRY_NAME}: ${${ENTRY_NAME}.name}")
        endforeach ()
    endif ()
endfunction()
