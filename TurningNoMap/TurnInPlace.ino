 ...
 * @param dir The direction of the rotation. Positive values rotate clockwise, negative values rotate counterclockwise.
 */
...
// Determines if the robot is on the line by counting the number of sensors detecting the line.
// Returns true if more than 2 sensors detect the line, false otherwise.
bool onLine()
{
    int n = 0;
    for (int i = 0; i < 10; i++)
...
/**
 * Moves the robot forward or backward based on the output value.
 * @param output The output value that determines the direction and speed of the movement.
 */
...
/**
 * Stops the motors by setting all pins to LOW and PWM values to 0.
 */
void stop()
{
    digitalWrite(PINMOTOR1A, LOW);
...
/**
 * Moves the robot according to a predefined array of stops.
 * Stops can be of different types:
 * 0 - continue moving forward until obstacle detected
 * 1 - rotate in place to the left until no obstacle in front
 * 2 - rotate in place to the right until no obstacle in front
