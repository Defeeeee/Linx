    ...
    { // mientras no se hayan completado todas las paradas
        ...
        while (moving)
        {
            ...
            { // cada 5 iteraciones, comprueba si se debe detener
                if (stop_sensor[stop_index] == 1 && Ult.checkF(50)) // si el sensor frontal detecta un obstaculo a menos de 50cm
                {
                    moving = false;
                }
                else if (stop_sensor[stop_index] == 2 && Ult.checkR(wall_distance)) // si el sensor derecho detecta un obstaculo a menos de 50cm
                {
                    moving = false;
                }
                l = 0;
